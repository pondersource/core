...
abstract class AbstractFederatedShareProvider implements IShareProvider {
	...
	public function create(IShare $share) {
		 ...
		 */
		$alreadyShared = $this->getSharedWith($shareWith, $this->shareType, $share->getNode(), 1, 0);
	...
	private function addShareToDB($itemSource, $itemType, $shareWith, $sharedBy, $uidOwner, $permissions, $expiration, $token) {
		...
		$qb->insert($this->shareTable)
			->setValue('share_type', $qb->createNamedParameter($this->shareType))
	...
	public function getChildren(IShare $parent) {
		...
		$qb->select('*')
			...
			->andWhere($qb->expr()->eq('share_type', $qb->createNamedParameter($this->shareType)))
			->orderBy('id');
	...
	public function getAllSharedWith($userId, $node) {
		return $this->getSharedWith($userId, $this->shareType, $node, -1, 0);
	}
	...
	public function getSharedWith($userId, $shareType, $node, $limit, $offset) {
		...
		$qb->where($qb->expr()->eq('share_type', $qb->createNamedParameter($this->shareType)));
